...
#from pytorch_kinematics import pytorch_kinematics as pk
...
print(torch.cuda.is_available())  # Trueが返ってくるべきです

# GPUが利用可能か確認し、必ずGPUを使う
if torch.cuda.is_available():
    ...
    raise RuntimeError("No GPU available, please ensure you have a CUDA-compatible GPU installed.")

# 精度の指定（GPU専用の場合もfloat32が一般的）
...
robot_urdf = "youbot.urdf"
name_end_effector = "pen_joint" # "arm_left_tool_link"
...
joint_pos_min = torch.tensor(
    [-2.95, -1.13, -2.55, -1.79, -2.89], dtype=dtype, device=d)
joint_pos_max = torch.tensor(
    [+2.95, +1.31, +2.55, +1.79, +2.89],  dtype=dtype, device=d)
...
########################################################################################
cartesian_res = 0.08 # metres 0.05 //
#####cartesian_resのサイズが小さいほどマップがより詳細になる. 値が大きいほど計算に必要なメモリが減少する.
# Set these as per your robot links)
########################################################################################
x_lim = [-1, 1.2] #[-1.0, 1.0] # min,max in metres (Set these as per your robot links)
...
for i in range(num_loops):
    ...
    indices_6d = indices_6d.cpu() # Copy to CPU GPU
    poses_6d = poses_6d.cpu() # Copy to CPU GPU
    del ee_transf_batch
    torch.cuda.empty_cache()
