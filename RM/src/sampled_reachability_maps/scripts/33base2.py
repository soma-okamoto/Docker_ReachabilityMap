...
###goal_pose = torch.tensor([0.75,0.2,0.55,0.1,np.pi/2,np.pi/4]) # euler angles in INTRINSIC ZYX (radians) TODO: Add quaternion support

goal_pose = torch.tensor([1,-1.2,0.2,0,90,0])#円になる
#goal_pose = torch.tensor([1,-1.2,0.2,200,0,0])#台形型
#goal_pose = torch.tensor([1,-1.2,0.2,0,0,200])
#goal_pose = torch.tensor([1, -1.2, 0.1, 20, 10, 0])
#goal_pose = torch.tensor([3,-1.2,0.4,0.1,0.3,0.5])
