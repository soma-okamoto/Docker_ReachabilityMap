...
goal_pose = torch.tensor([0.04,-0.5,0.3,0.,np.pi/4,np.pi/3])
#goal_posetorch.tenssor(]0.75,0.2,0.55,0.,np.pi/4,np.pi/3)0,2.2,-0.4,0,0,0
# Choose float32 or 64 etc.
...
with open(args.inv_map_pkl,'rb') as f:
    ...
    if ("Vis_freq" in loaded_dict.keys()) and use_vis_freq:
        ...
        Manip_scaling = 500000000000000000000000000000000
    else:
        Manip_scaling = 1000000000000000000000000000000000
